...
export default function LinkCard({ label, url, icon, count, onClick }: LinkCardProps) {
  return (
    <a
      href={url}
      target="_blank"
    ...
    >
      ...
      <span className="flex-1 font-medium text-[#4a3728]">{label}</span>
      <span className="shrink-0 text-xs text-[#8a7160]">{count}회</span>
    </a>
  );
}
